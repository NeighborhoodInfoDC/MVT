...
%let acs_vars = 
	...
	%let birth_vars =births_total_2003 births_total_2004 births_total_2005 births_total_2006
		...
		%let price_vars = mprice1995 mprice1996 mprice1997 mprice1998
				mprice1999 mprice2000 mprice2001 mprice2002
...
data compile_mvt_tabs_target;
	set compile_mvt_tabs_target;
	geography = "Census Tract 47.01+47.02";
	target = 1;
run;
...
data compile_mvt_tabs_target_adj;
	set compile_mvt_tabs_target_adj;
	geography = "Census Tract 47 and Surrounding Tracts";
	target = 2;
run;
...
proc tabulate data = sales_res out = price_tab_target;
class target saleyear;
var saleprice ;
table target, saleyear *(median) *saleprice;
run;
...
data compile_mvt_tabs_t;
	set compile_mvt_tabs_target compile_mvt_tabs_target_adj;
run;

data compile_mvt_tabs_t;
	merge compile_mvt_tabs_t price_target (keep= target &price_vars);
	by target;
run;
...
data compile_mvt_tabs;
	set compile_mvt_tabs_t compile_mvt_tabs_wd12_select compile_mvt_tabs_city;
		...
		/*Race and ethnicity*/
			...
			PctOth&_years. = popotherracenonhispbridg&_years. / popwithrace&_years.;
...
proc transpose data=compile_mvt_tabs out=mvt_tabs(label="Bridge Park Tabulations, &geo");
