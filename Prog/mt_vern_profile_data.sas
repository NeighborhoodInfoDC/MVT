...
data compile_mvt_tabs_&geosuf;
	merge	ACS.acs_2012_16_dc_sum_tr_&geosuf
		...
		NCDB.Ncdb_sum_&geosuf
			(keep= &geo totpop_1990 totpop_2000 popwithrace_2010 popasianpinonhispbridge_1990 popblacknonhispbridge_1990 
...
data compile_mvt_tabs_tr10;
	set compile_mvt_tabs_tr10;
	if geo2010 in ('47', list of tracks) then target = 1;
	if geo2010 in ('47', list of tracks) then target_adj = 1;
run;
...
data compile_mvt_tabs_wd12;
	...
	if geo2010 not in (list of wards) then delete;
run;
